...
class TrigramLMWithSmoothing(BigramLM.BigramLM):
    def __init__(self, sentences, k, l1, l2, l3, unk_threshold):
    ...
    def validate(self):
        for key in self.trigram_frequencies.keys():
            ...
            for word3 in self.trigram_frequencies[key].keys():
                probability_sum += self.__calculate_probability(key[0], key[1], word3)
    ...
    def calculate_perplexity(self, sentences):
        ...
        for sentence in sentences:
            ...
            for key in keys:
                ...
                trigram_word_probability = self.__calculate_probability(key[0], key[1], key[2])
    ...
    def __calculate_probability(self, previous_previous_word, previous_word, word):
        return self.l1 * self.__calculate_trigram_probability(previous_previous_word, previous_word, word) + \
               self.l2 * self.__calculate_bigram_probability(previous_word, word) + \
               self.l3 * self.__calculate_unigram_probability(word)
    ...
    def __calculate_bigram_probability(self, previous_word, word):
        if previous_word in self.bigram_frequencies:
            word_probability_denominator = \
                sum(self.bigram_frequencies[previous_word].values()) + \
                len(self.unigram_frequencies.keys()) * self.k
            word_probability_numerator = self.bigram_frequencies[previous_word].get(word, 0) + self.k
        else:
            word_probability_denominator = len(self.bigram_frequencies.keys()) * self.k
            word_probability_numerator = self.k
