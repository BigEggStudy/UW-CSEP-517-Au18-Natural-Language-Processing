...
class TrigramHMM:
    def __init__(self, k1, k2, l1, l2, l3):
        self.k1 = k1
        self.k2 = k2
        self.l1 = l1
        self.l2 = l2
    ...
    def __get_transition_prob_bigram(self, transition_data):
        ...
        if prevTag2 not in self.bigram_tag_frequencies:
            return 0.0
        return float(self.bigram_tag_frequencies[prevTag2][currTag] + self.k2) / float(sum(self.bigram_tag_frequencies[prevTag2].values()) + (self.k2 * len(self.bigram_tag_frequencies.keys())))

    def __get_transition_prob_trigram(self, transition_data):
        prevTag1, prevTag2, currTag = transition_data

        if (prevTag1, prevTag2) not in self.trigram_tag_frequencies:
            return float(self.k2) / float(self.k2 * len(self.trigram_tag_frequencies.keys()))
        return float(self.trigram_tag_frequencies[(prevTag1, prevTag2)][currTag] + self.k2) / float(sum(self.trigram_tag_frequencies[(prevTag1, prevTag2)].values()) + (self.k2 * len(self.trigram_tag_frequencies.keys())))
